...
class BlenderNeRF_Operator(bpy.types.Operator):
    ...
    def get_camera_extrinsics(self, scene, camera, mode="TRAIN", method="SOF"):
        ...
        step = (
            scene.train_frame_steps
            if (mode == "TRAIN" and method == "SOF")
            else scene.frame_step
        )
        ...
        for frame in range(scene.frame_start, end + 1, step):
            ...
            frame_data = {
                "file_path": os.path.join(
                    filedir, os.path.splitext(filename)[0] if scene.splats else filename
                ),
        ...
        for obj in scene.objects:
            if obj.type == "MESH" and self.is_object_visible(obj):
                if (
                    ...
                    and TMP_VERTEX_COLORS in obj.data.vertex_colors
                ):
                    obj.data.vertex_colors.remove(
                        obj.data.vertex_colors[TMP_VERTEX_COLORS]
                    )
    ...
    def asserts(self, scene, method="SOF"):
        ...
        if method == "TTC" and not (
            train_camera.data.type == "PERSP" and test_camera.data.type == "PERSP"
        ):
            error_messages.append("Only perspective cameras are supported!")
        ...
        if method == "COS" and CAMERA_NAME in scene.objects.keys():
            ...
            if not sphere_camera.data.type == "PERSP":
                error_messages.append(
                    "BlenderNeRF Camera must remain a perspective camera!"
                )

        if (
            (method == "SOF" and sof_name == "")
            or (method == "TTC" and ttc_name == "")
            or (method == "COS" and cos_name == "")
        ):
            error_messages.append("Dataset name cannot be empty!")

        if method == "COS" and any(x == 0 for x in scene.sphere_scale):
            error_messages.append(
                "The BlenderNeRF Sphere cannot be flat! Change its scale to be non zero in all axes."
            )

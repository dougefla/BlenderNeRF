...
class TrainTestCameras(blender_nerf_operator.BlenderNeRF_Operator):
    ...
    def execute(self, context):
        ...
        if scene.test_data:
            # testing transforms
            output_test_data["frames"] = self.get_camera_extrinsics(
                scene, test_camera, mode="TEST", method="TTC"
            )
            self.save_json(output_path, "transforms_test.json", output_test_data)

        if scene.train_data:
            # training transforms
            output_train_data["frames"] = self.get_camera_extrinsics(
                scene, train_camera, mode="TRAIN", method="TTC"
            )
            ...
            if scene.render_frames:
                ...
                scene.frame_end = (
                    scene.frame_start + scene.ttc_nb_frames - 1
                )  # update end frame
                scene.render.filepath = os.path.join(
                    output_train, ""
                )  # training frames path
